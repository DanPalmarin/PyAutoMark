...
class Ui_MainWindow(object):
    ...
    def button3_handler(self):
        ...
        for zip_file in self.button2_list:
            ...
            if zip_file not in self.checked:
                ...
                self.output_file = "{0}\\{1}{2}.txt".format(self.output_dir, word1, student_name)
                ...
                for i in range(1,len(solMod.Q_all)+1):
                    for entry in os.scandir(self.extracted_dir):
                        if "A{0}_{1}.".format(solMod.assignment_num, i) in entry.path:
    ...
    def question(self, Q_num, file, inp, outp, outfile, weight):
        ...
        if inp == []:
            result = self.running_no_input_assignments(trim_tail)
            ...
            for i in range(len(inp)):
                if type(inp[i]) is tuple:
                    ...
                    result = self.running_with_inputs_assignments(trim_tail, formatted_input)
                else:
                    result = self.running_with_inputs_assignments(trim_tail, str(inp[i]))
    ...
    def running_no_input_assignments(self, assignment):
    ...
    def running_with_inputs_assignments(self, assignment, input_feed):
        output_feed = StringIO()
        with redirect_stdout(output_feed):
